...
FSElement::FSElement(std::shared_ptr<FSElement> parent, const std::string& name,
    ...
    : parent_(parent),
      ...
      size_(0) {
  if (nullptr == parent) assert(name == "/"); // TBD: should these be runtime errors.
  else assert(false == parent->isFile());
};

std::shared_ptr<FSElement> FSElement::getParent() const { return parent_; }
...
void FSElement::setModTime(std::time_t& last_modified) { last_modified_ = last_modified; }
