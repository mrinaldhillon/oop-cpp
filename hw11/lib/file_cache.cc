...
#include <memory>
#include "lib/fifo.h"
#include "lib/null_replacement.h"
#include "lib/file.h"
...
FileCache::FileCache() : replacement_policy_(&NullReplacement::getInstance()) {}
...
std::string FileCache::fetch(const std::string& target_file) {
  try {
    ...
    if (cache_.size() < cache_threshhold_) {
      replacement_policy_->insert(cache_, file);
    } else {
      replacement_policy_->replace(cache_, file);
    }
    return data;
  }
}
